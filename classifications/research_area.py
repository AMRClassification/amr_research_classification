...
def classify_research_area(title, abstract, model="gpt-4o-mini"):
    ...
    while tries < max_tries:
        try:
            ...
            if has_therapeutics:
                validation_result = validate_therapeutics_classification(
                    title, abstract, str(result)
                )

                if validation_result:
                    if not validation_result["is_correct"]:
                        print("\nResearch Area Classification Validation Failed!")
                        ...
                        print("\nResearch Area Classification Validated Successfully!")
                        parsed_result["explanation"] = (
                            parsed_result["explanation"]
                            + "\n\nValidation Result:\n"
                            + validation_result["explanation"]
